...
//    NETWORK_init();
        ...
        graphics->backBuffer = SDL_CreateRGBSurface(SDL_SWSURFACE, 320, 240, 32, fmt->Rmask, fmt->Gmask, fmt->Bmask,
                                                   fmt->Amask);
        SDL_SetSurfaceBlendMode(graphics->backBuffer, SDL_BLENDMODE_NONE);
        graphics->Makebfont();


        graphics->forgroundBuffer = SDL_CreateRGBSurface(SDL_SWSURFACE, 320, 240, fmt->BitsPerPixel, fmt->Rmask, fmt->Gmask,
                                                        fmt->Bmask, fmt->Amask);
        SDL_SetSurfaceBlendMode(graphics->forgroundBuffer, SDL_BLENDMODE_NONE);

        graphics->screenbuffer = gameScreen;

        graphics->menubuffer = SDL_CreateRGBSurface(SDL_SWSURFACE, 320, 240, fmt->BitsPerPixel, fmt->Rmask, fmt->Gmask,
                                                   fmt->Bmask, fmt->Amask);
        SDL_SetSurfaceBlendMode(graphics->menubuffer, SDL_BLENDMODE_NONE);

        graphics->towerbuffer = SDL_CreateRGBSurface(SDL_SWSURFACE, 320, 240, fmt->BitsPerPixel, fmt->Rmask, fmt->Gmask,
                                                    fmt->Bmask, fmt->Amask);
        SDL_SetSurfaceBlendMode(graphics->towerbuffer, SDL_BLENDMODE_NONE);

        graphics->tempBuffer = SDL_CreateRGBSurface(SDL_SWSURFACE, 320, 240, fmt->BitsPerPixel, fmt->Rmask, fmt->Gmask,
                                                   fmt->Bmask, fmt->Amask);
...
void main_loop() {
    {
        if(context == nullptr){
            context = new context_t(argc_, argv_);
        }

        Screen &gameScreen = *context->gameScreen;
        UtilityClass &help = *context->help;
        Graphics &graphics = *context->graphics;
        musicclass &music = *context->music;
        Game &game = *context->game;
        KeyPoll &key = *context->key;
